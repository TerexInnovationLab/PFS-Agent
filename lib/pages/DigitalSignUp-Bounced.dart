...
class DigitalSignUpBouncedState extends State<DigitalSignUpBounced> {
  ...
  Map<String, dynamic> _collectFormData() {
    return {
      ...
      'maritalStatusSelected': maritalStatus != null,
      ...
      'employedFullTimeSelected': employedFullTime != null,
      ...
      'salaryFrequencySelected': salaryFrequency != null,
      ...
      'salaryPaidIntoAccountSelected': salaryPaidIntoAccount != null,
      ...
      'salaryTransferred3MonthsSelected': true,
  ...
  void _loadFromData(Map<String, dynamic> data) {
    setState(() {
      ...
      if (identificationPathBack != null &&
  ...
  Future<void> sendOnboarding({String? referralCode}) async {
    ...
    try {
      ...
      request.fields.addAll({
        ...
        "loan_purpose": ?loanpurpose.toString() == "Other"
            ? loanPurposeController.text.toString()
            : loanpurpose,
    ...
    switch (_currentStep) {
      ...
      case 0:
        ...
        if (traditionalAuthorityController.text.trim().isEmpty)
          missing.add('Traditional Authority');
      ...
      case 1:
        if (physicalAddressController.text.trim().isEmpty)
          missing.add('Physical address');
      ...
      case 2:
        ...
        if (mobile1Controller.text.trim().isEmpty)
          missing.add('mobile Tel no 1');
        break;

      // 3 => Family Member (_familyMemberStep)
      case 3:
        if (famSurnameController.text.trim().isEmpty)
          missing.add('Family member surname');
        if (famFirstNameController.text.trim().isEmpty)
          missing.add('Family member first name');
        if (famRelationController.text.trim().isEmpty)
          missing.add('Family member relation');
        //if (famHomeTelController.text.trim().isEmpty) missing.add('Family member home tel. no');
        if (famMobileController.text.trim().isEmpty)
          missing.add('Family member mobile tel. no');
        if (famAddressController.text.trim().isEmpty)
          missing.add('Family member address');
      ...
      case 4:
        ...
        if (employerCodeController.text.trim().isEmpty)
          missing.add('Employer code / payroll no');
      ...
      case 6:
        if (clientSignaturePath == null || clientSignaturePath!.isEmpty)
          missing.add('Client signature file');
        if (identificationPath == null || identificationPath!.isEmpty)
          missing.add('Front Identification file');
        if (identificationPathBack == null || identificationPathBack!.isEmpty)
          missing.add('Back Identification file');
        ...
        if (customerPhoto == null || customerPhoto!.isEmpty)
          missing.add('Customer Photo file');
      ...
      case 7:
        ...
        if (totalAppliedController.text.trim().isEmpty)
          missing.add('Total Applied for');
        break;
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(
                'Applicant: ${firstNameController.text} ${surnameController.text}',
              ),
              Text('ID: ${idNumberController.text}'),
              Text('Email: ${email1Controller.text}'),
              Text(
                'Application Date: ${applicationDate?.toLocal().toIso8601String() ?? '-'}',
              ),
            ],
          ),
        ),
      ...
      children: [
        ...
        if (isLast)
          Expanded(
            child: ElevatedButton(
              onPressed: _sending
                  ? null
                  : _onReferralPressed, // ⬅ disable when sending
              child: const Text(
                "Referral",
        ...
        Expanded(
          child: ElevatedButton(
            ...
            child: Text(
              isLast ? (_sending ? 'Sending...' : 'Submit') : 'Next',
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // SizedBox(height: 20),
            // TextFormField(
            //   controller: totalApprovedController,
            //   decoration: InputDecoration(
            //     labelText: 'Total Amount Approved',
            //     border: OutlineInputBorder(
            //       borderSide: BorderSide.none,
            //       borderRadius: BorderRadius.circular(12),
            //     ),
            ...
            // SizedBox(height: 20),
            // TextFormField(
            //   controller: cashToClientController,
            //   decoration: InputDecoration(
            //     labelText: 'Cash to Client',
            //     border: OutlineInputBorder(
            //       borderSide: BorderSide.none,
            //       borderRadius: BorderRadius.circular(12),
            //     ),
            ...
            DropdownButtonFormField<String>(
              ...
              items:
                  ['Education', 'Housing', 'Debt Payment', 'Funeral', 'Other']
                      .map(
                        (t) => DropdownMenuItem(
                          value: t.toLowerCase(),
                          child: Text(t),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (hasFile)
            GestureDetector(
              onTap: () => _openImagePreview(File(filePath!)),
              child: ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: Image.file(
                  File(filePath!),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  _reviewImageRow('Bank Statement', bankStatementPath),
                  _reviewImageRow('Employer Letter', employerLetterPath),
                ],
              ),
            ),
          ),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  _reviewRow(
                    'Base Lending Rate (%)',
                    baseLendingRateController.text,
                  ),
                  _reviewRow(
                  ...
                  _reviewRow(
                    ...
                    _fmtDate(firstInstalmentDate),
                  ),
                  _reviewRow(
                    'Last Instalment Date',
                    _fmtDate(lastInstalmentDate),
                  ),
                  _reviewRow('Net Pay', netPayController.text),
                  _reviewRow(
                    'Max Allowed Instalment',
